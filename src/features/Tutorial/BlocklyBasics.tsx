...
export default function BlocklyBasics() {
  ...
  const modalText = [
    'Lets get into how to use Blockly to move our robot! From the "Actions" group, try dragging the "Set linear speeds" block into the coding area',
    'Great! Now lets set the x speeds to 3',
    'One last step! Drag the "Execute" block from Actions to make this code execute!',
    'Our robot moves now! But it keeps running into the wall! Lets fix this by making use of the robots sensors',
    'The robots forward sensors will activate when it runs into a wall facing forward, so lets use that to change the robots orientation! First, drag an "if/do" block from the Logic group to connect underneath the linear speed block',
    ...
    'Awesome work! The robot now moves around the simulator! You can now try making use of the robots other sensors to see how they affect the robots movement or click next to move onto the Clustering configuration tutorial'
  ];
  return (
    <div>
      <Tutorial
